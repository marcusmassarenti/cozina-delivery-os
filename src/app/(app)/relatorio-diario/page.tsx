...
import { DailyMatrix } from "./_components/daily-matrix"
...
export default async function RelatorioDiarioPage({
  ...
  const bars: Bar[] = matrix.days.map((d) => {
    const v = dayValue(d)
    return { day: d, value: v, label: fmt(v) }
  })
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 bg-muted/30 p-6">
      ...
      {!matrix.hasData ? (
        ...
        <>
          <RelatorioKpis matrix={matrix} />
          <DailyBarChart bars={bars} metric={metric} metricLabel={metricLabel} />
          <UnitsRanking
            units={rankingUnits}
            metric={metric}
            metricLabel={metricLabel}
          />
          <DailyMatrix matrix={matrix} metric={metric} />
        </>
      )}
    </div>
  )
