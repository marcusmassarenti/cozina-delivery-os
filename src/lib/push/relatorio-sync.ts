 ...
 *
 * O que este push NÃO cobre, e por isso o `resumo-importacao` (6h30) continua
 * existindo: cron que não roda não manda nada. Silêncio aqui é ambíguo — pode
 * ser "deu tudo certo e o aparelho estava sem sinal" ou "a rotina nunca
 * disparou". Quem desfaz essa ambiguidade é o fechamento, lendo `cron_runs`.
 */
import "server-only"
...
export async function enviarRelatorioSync(
  r: RelatorioSync,
): Promise<{ enviados: number; destinatarios: number; titulo: string; corpo: string }> {
  let titulo: string
  let corpo: string
