...
async function main() {
  ...
  const holdingDaBrand = new Map((bs ?? []).map((b: any) => [b.id, b.holding_id]))
  const nomeHolding = new Map((hs ?? []).map((h: any) => [h.id, h.nome]))
  const units = (us ?? []).filter((u: any) => u.active)
  const ids = units.map((u: any) => u.id)

  const [monthly, metrics, fin, cestas] = await Promise.all([
    getRealMonthlyForUnits(ids, Y, M),
    getUnitMetricsForMonth(ids, ["ifood", "99food", "keeta", "cardapioweb"] as any, Y, M),
  ...
  for (const u of units) {
    const h = nomeHolding.get(holdingDaBrand.get(u.brand_id)) ?? "(sem holding)"
  ...
  let T = { agg: 0, evo: 0, portal: 0, cesta: 0 }
